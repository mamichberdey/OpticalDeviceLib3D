...
class OpticalDevice(ABC):
    ...
    @staticmethod
    def gauss_filter_2d(arr, x, y, sig_x, sig_y, core='gauss'):
        ...
        if core == 'gauss':
            core = np.exp(-(x/sig_x)**2/2 - (y/sig_y)**2/2) / (2 * np.pi * sig_x * sig_y)
        elif core == 'lorentz':
            core = 1 / (2 * np.pi * sig_x * sig_y * (1 + (x/sig_x)**2 + (y/sig_y)**2)**(3/2))


        fft = OpticalDevice.ft2d
        ifft = OpticalDevice.ift2d
        ans = ifft(array=fft(arr, dx, dy)*fft(core, dx, dy), dx=dx, dy=dy)
        return ans.real
...
class PointSource(OpticalDevice):
    ...
    def __init__(self, z, x0, y0, En=None, lam=None) -> None:
        super().__init__()
    ...
    def E(self):
        return super().P(x=self.x-self.x0, y=self.y-self.y0, z=np.sqrt(self.z**2+self.x0**2+self.y0**2), k=self.k)
...
class CRL2D(OpticalDevice):
    ...
    def focus_params(self, z_arr, T_arr=None, nx_cut=50, ny_cut=50, focus=None):
        ...
        cx_end = np.min([self.Nx//2 + nx_cut, self.Nx])
        cy_start = np.max([self.Ny//2 - ny_cut, 0])
        cy_end = np.min([self.Ny//2 + ny_cut, self.Ny])
        s1, s2 = int(cx_end - cx_start), int(cy_end - cy_start)
        focus_images = cp.empty(shape=(s0, s2, s1), dtype=complex)

        for i, z_i in enumerate(z_arr):
            E_arr = self.sv(arr=E0, z=z_i, k=self.k)
            I_arr = abs(E_arr) ** 2
            focus_images[i] = E_arr[cy_start:cy_end, cx_start:cx_end]
        ...
        return I_distr_max_cutted, float(I_max), float(dz_max)*1e3, dy_max*1e6, dx_max*1e6, float(fwhm_y), float(fwhm_x),\
              focus_images, z_arr.get(), div_ang_x, div_ang_y, x_cut, y_cut
...
class CRL1D(CRL2D):
    ...
    def T(self, x=None, y=None, R=None, A=None, d=None, lt=None):
        ...
        # if isinstance(x, np.ndarray):
        y = np.multiply(y, mask)
        par = y**2/(2*R) + anti_mask*h_max
        profile_surface = np.minimum(par, h_max) - h_max
        # profile_surface_filtered = self.od_1d.OpticalDevice.gauss_filter_1d(profile_surface, sigma=20e-6, xx=self.x[0])
        # profile_surface_filtered = self.od_1d.OpticalDevice.gauss_filter_1d(profile_surface_filtered , sigma=5e-6, xx=self.x[0], dx=self.dx, axis=0)
        # return np.minimum(par, h_max) + d/2
        return profile_surface.real + d/2 + h_max
...
class CRL1Deq(CRL1D):
    ...
    def __init__(self, z, arr_start, R, A, d, N_lens, mol, dens, copy, dist_l, dist_r, lt, is_dist_const, lam=None, En=None):
        self.lam, self.En = self.wavelength_energy(lam, En)
        self.w, self.k = 2*self.c*np.pi/self.lam, 2*np.pi/self.lam
        self.mol, self.dens = mol, dens
        self.delta, self.betta = MolPar.chi0h(energy=self.En, molecula=self.mol, ro=self.dens)
        ...
        self.z = z
        self.lt = lt
        self.dist_l, self.dist_r = dist_l, dist_r
    ...
    def T(self, x=None, y=None, R=None, A=None, d=None, dist_l=None, dist_r=None, lt=None, is_dist_const=None):
        """ Returns CRL's thickness"""
        x = self.x if x == None else x
        y = self.y if y == None else y
        R = self.R if R == None else R
        ...
        d = self.d if d == None else d
        lt = self.lt if lt == None else lt
        dist_l = self.dist_l if dist_l == None else dist_l
        dist_r = self.dist_r if dist_r == None else dist_r
        ...
        for i, x_C in enumerate(x[0]):
            dist_new = float(dist_l*(x_r-x_C)/(x_r-x_l)) + float(dist_r*(x_l-x_C)/(x_l-x_r))
            par[:, i] = self.parallel_curve_arr(x_arr=y[:, 0], a=1/(2*R), dist=dist_new, eps=1e-20) - dist_new * is_dist_const # parallel curve d
            
        
        dist_gs = min(dist_l, dist_r) * (1 - is_dist_const)
        par = np.where(mask, par, h_max - dist_gs) # put the last param as h_max if you want invisible wall around the CRL

        # profile_surface = np.minimum(par, h_max) - h_max
        # profile_surface_filtered = self.od_1d.OpticalDevice.gauss_filter_1d(profile_surface, sigma=5e-7, xx=self.x[0])
        # return profile_surface_filtered.real + by + d/2 + h_max
        return np.minimum(par, h_max - dist_gs) + dist_gs + d/2

class CRL1Dcurv(CRL1D):

    def __init__(self, z, arr_start, R, A, d, N_lens, mol, dens, copy, lt, ex_l, ex_r, lam=None, En=None):
        self.lam, self.En = self.wavelength_energy(lam, En)
        self.w, self.k = 2*self.c*np.pi/self.lam, 2*np.pi/self.lam
        self.mol, self.dens = mol, dens
        self.delta, self.betta = MolPar.chi0h(energy=self.En, molecula=self.mol, ro=self.dens)
        ...
        if ex_l == "min":
            self.ex_l = self.ex_min
        elif ex_l == "max":
            self.ex_l = self.ex_max
        else:
            self.ex_l = ex_l

        if ex_r == "min":
            self.ex2 = self.ex_min
        elif ex_r == "max":
            self.ex_r = self.ex_max
        else:
            self.ex_r = ex_r
    ...
    def T(self, x=None, y=None, R=None, A=None, d=None, ex_l=None, ex_r=None, lt=None):
        """ Returns CRL's thickness"""
        x = self.x if x == None else x
        y = self.y if y == None else y
        R = self.R if R == None else R
        ...
        d = self.d if d == None else d
        lt = self.lt if lt == None else lt
        ex_l = self.ex_l if ex_l == None else ex_l
        ex_r = self.ex_r if ex_r == None else ex_r
        ...
        for i, x_C in enumerate(x[0]):
            ex_new = float(ex_l*(x_r-x_C)/(x_r-x_l)) + float(ex_r*(x_l-x_C)/(x_l-x_r))
            par[:,i] = self.ellipse(x=y[:, 0], ex=ex_new, A=A, H=h_max)
            # par[:,i] = self.parallel_curve_arr(x_arr=y[:, 0], a=1/(2*R), d=d_new, eps=1e-20) # parallel curve d
        par = np.where(mask, par, h_max) # put the last param as h_max if you want invisible wall around CRL
...
class CRL1Dm(CRL1D):
    ...
    def __init__(self, z, arr_start, R, A, d, N_lens, mol, dens, copy, dist_l, dist_r, lt, shift, sigma, sigma_c, height, height_c, lam=None, En=None):
        self.lam, self.En = self.wavelength_energy(lam, En)
        self.w, self.k = 2*self.c*np.pi/self.lam, 2*np.pi/self.lam
        self.mol, self.dens = mol, dens
        self.delta, self.betta = MolPar.chi0h(energy=self.En, molecula=self.mol, ro=self.dens)
    ...
    def T(self, x=None, y=None, R=None, A=None, d=None, dist_l=None, dist_r=None, lt=None, shift=None, height=None, height_c=None, sigma=None, sigma_c=None):
        """ Returns CRL's thickness"""
        x = self.x if x == None else x
        y = self.y if y == None else y
        R = self.R if R == None else R
        ...
        #     # coef = 1 / (sig * np.pi)
        #     # coef = 1
        #     out = 1 / (1 + ((x-mean)/sig)**2)
        #     out /= np.max(out)
        #     return coef * out
        
        # def norm_pdf(x, sig, mean, coef):
        #     out = 1 / R / (1+(sig*(x-mean)/R)**2)**(3/2)
        #     out /= np.max(out)
        #     return coef * out
        ...
        for i, x_C in enumerate(x[0]):
            dist_new = float(dist_l*(x_r-x_C)/(x_r-x_l)) + float(dist_r*(x_l-x_C)/(x_l-x_r))
            height_new = height * dist_new / height_c
            # height_c = height_c * dist_new / height
            arr_gauss_l = norm_pdf(y[:, 0], sig=sigma, mean=shift, coef=height_new) # задаем веса
            arr_gauss_r = norm_pdf(y[:, 0], sig=sigma, mean=-shift, coef=height_new)
            arr_gauss_c = norm_pdf(y[:, 0], sig=sigma_c, mean=0, coef=dist_new)
            # coef = 4
            # y_cross_val = y_cross(R2=coef*R, l=height_new, R=R)            
            # par[:, i] = (y[:, 0]**2)/(2*R)
            # par[:, i][par[:, i]<y_cross_val] = circle(x=y[:, 0][par[:, i]<y_cross_val], R2=coef*R, l=height_new)

            par[:, i] = (y[:, 0]**2)/(2*R) - arr_gauss_l - arr_gauss_r - arr_gauss_c
            # par[:, i] = (y[:, 0]**2)/(2*R) - arr_gauss_c
            
        
        dist_gs = min(dist_l, dist_r)
        par = np.where(mask, par, h_max - dist_gs) # put the last param as h_max if you want invisible wall around the CRL
